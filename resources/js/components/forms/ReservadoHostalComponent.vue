<template>
         <div class="">
            <form id="reservated" class="mt-5 pt-3 pb-5 bg-primary" hidden="true">
            ...
            <div class="container mt-5">  <h1 class="text-center text-primary font-weight-bold mt-5 mb-5">{{ $trans('messages.Books') }}</h1>
              <div class="alert alert-info" v-if="mensage!=''">
                <ul>
                  <li>{{mensage}}</li>
                </ul>
              </div>
              <div class="card shadow mb-4 mt-3" v-for="(res,index) in reservas" :res="res" :key="res.id">
                <div class="card-header py-3 row">
                  <div class="col-11">
                  <h6 class="m-0 font-weight-bold text-primary">{{res.name}}</h6>
                  </div>
                  <div class="col-1">
                    <a href="#" @click="deleteReservation(index,res)"><i class="fa fa-trash-alt"></i></a>
                  </div>
                </div>
                <div class="card-body row">
                  ...
                  <div class="col-2"><h5>{{ $trans('messages.Childs') }}</h5>
                    <p>{{res.child}}</p>
                  </div>
                ...
                <div class="card-footer row">
                  <div class="col-6">
                    <h4>{{ $trans('messages.Date In') }}</h4>
                    <p>{{res.date_in}}</p>
                  </div>
                  <div class="col-6">
                    <h4>{{ $trans('messages.Date Out') }}</h4>
                    <p>{{res.date_out}}</p>
                  </div>
                </div>
              </div>
...
import moment from 'moment';
    export default {
      ...
      methods:{
        deleteReservation:function(index,reservation){
          let reserva_id=reservation;
            swal({title:this.$trans('messages.Delete Reservation'),
                  ...
                  if (select){
                    let  url='/delete-book-by-guest/'+reserva_id['id']+'/'+this.email+'/'+reserva_id['name'];
                    axios.post(url)
                         .then(response=>{
                         ...
                         .catch(error=>{
                           ...
                           let newMessage=this.$trans('messages.You have just deleted a reservation, a notification must be send to your email. But in this moment we present this problem with the email send: ')+' '+message;
                             this.reservas.splice(index,1);
      ...
      getBooks:function(){
        ...
        axios.get(url)
             ...
             if (this.reservas.length===0){
               this.mensage=this.$trans('messages.None reservation added yet');
             }
